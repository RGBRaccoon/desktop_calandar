...
## T10 — 사용자 배경 이미지
- Goal / Reason: 로컬 JPG/PNG를 달력 배경으로 선택하고 재실행 후 유지.
- Scope: 이미지 검증/앱 폴더 저장, 비율 유지 채우기 렌더링, 밝기/투명도/기본 배경 설정, 테스트 및 v0.1.2 EXE.
- Acceptance Criteria: 원본 삭제 후에도 배경 유지, 취소 시 설정 미변경, 잘못된 이미지 거부, 테마/일정/드래그 유지, 이미지와 설정 EXE 검증.
- Tests: 이미지 저장 및 설정 UI/렌더링 회귀, Windows 100%/150% 전체 검사, 배포 EXE 상호작용 검사.
- Dependencies: T8/T9.
- 결과: JPG/PNG 검증과 앱 데이터 복사, 화면 채우기, 밝기·불투명도·제거 기능 구현. 교체·제거 시 관리 파일만 정리. Windows 100%/150%에서 전체 43개 테스트와 OAuth 포함 v0.1.2 EXE의 트레이/버튼/드래그/설정/배경 복사·재시작·실제 화면 픽셀 검증 통과.
